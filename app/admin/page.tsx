...
export default async function AdminHomePage() {
  try {
    ...
    return (
      <section style={{ padding: "40px", color: "#991b1b", background: "#fef2f2", minHeight: "100vh", fontFamily: "monospace" }}>
        <h1 style={{ fontSize: "24px" }}>❌ Admin Server Component Render Exception</h1>
        <p style={{ fontWeight: "bold" }}>{err?.message || "Unknown Message Error"}</p>
        <pre style={{ padding: "20px", background: "#fff", border: "1px solid #fee2e2", borderRadius: "8px", overflow: "auto" }}>
          {err?.stack || "No stack trace discovered."}
        </pre>
      </section>
    );
  }
